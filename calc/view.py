...
from .string_calc import calc_from_string

# yes they dont follow conventions
# but they are the best string input math library i can find so lets use em
...
class CalcView(discord.ui.View):
    ...
    async def calc_lazy_edit(self):
        ...
        while True:
            ...
            await asyncio.sleep(1)  # hopefully this is enough... 1.7 wait in total
    ...
    def maybe_update_output(self) -> bool:
        try:
            self.output = round(calc_from_string(self.input), 10)
            return True
        except SyntaxError:  # not finished typing
            return False
