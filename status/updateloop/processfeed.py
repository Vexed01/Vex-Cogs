...
from ..objects.incidentdata import IncidentData, UpdateField


def _handle_long_fields(old_fields: List[UpdateField]) -> List[UpdateField]:  # using updatefield because idk really
    """Split long fields (over 1024 chars) into multiple, retaining order."""
    new_fields = []
    for field in old_fields:
        field.value = re.sub(r"(\n\n\n)(\n)*", "\n\n", field.value)  # excessive new lines (looking at you cloudflare)
        if len(field.value) <= 1024:
            ...
            for page in paged[1:]:
                new_fields.append(UpdateField("Above continued (hit field limits)", page, field.update_id))

    return new_fields


def _process(incident: dict, type: str):
    fields = []
    for update in incident["incident_updates"]:
        # this is exactly how they are displayed on the website
        friendly_time = parse_time(update["created_at"]).astimezone(pytz.utc).strftime("%b %d, %H:%M %Z")

        fields.append(
            UpdateField(
                name="{} - {}".format(update["status"].replace("_", " ").capitalize(), friendly_time),
                value=update["body"],
                update_id=update["id"],
            )
        )
    ...
    if type == "scheduled":
        start = parse_time(incident["scheduled_for"]).astimezone(pytz.utc).strftime("%b %d, %H:%M %Z")
        end = parse_time(incident["scheduled_until"]).astimezone(pytz.utc).strftime("%b %d, %H:%M %Z")
...
def process_incidents(json_resp: dict):
    return [_process(j_data, "incidents") for j_data in json_resp.get("incidents", [])]


def process_scheduled(json_resp: dict):
    return [_process(j_data, "scheduled") for j_data in json_resp.get("scheduled_maintenances", [])]
